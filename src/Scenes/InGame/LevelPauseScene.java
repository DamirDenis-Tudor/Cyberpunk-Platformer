...
final public class LevelPauseScene extends Scene {
    ...
    public LevelPauseScene(SceneHandler sceneHandler) {
        super(sceneHandler);
        components.add(AssetDeposit.get().getMenuImage(MENU_WALLPAPER));
    ...
    public void notify(Message message) {
        switch (message.type()) {
            case BIKER_SELECTED -> {
                ...
                playerPreview = new Animation(AssetDeposit.get().getAnimation(AnimationType.BikerIdle));
            ...
            case PUNK_SELECTED -> {
                selectedPlayer = PUNK;
                ((Notifiable) components.get(1)).notify(new Message(MessageType.CLEAR_INVENTORY, SCENE, -1));
                playerPreview = new Animation(AssetDeposit.get().getAnimation(AnimationType.PunkIdle));
                playerPreview.setPosition(new Coordinate<>(1400, 300));
                playerPreview.setAnimationScale(5);
            }
            case CYBORG_SELECTED -> {
                ...
                ((Notifiable) components.get(1)).notify(new Message(MessageType.CLEAR_INVENTORY, SCENE, -1));
                playerPreview = new Animation(AssetDeposit.get().getAnimation(AnimationType.CyborgIdle));
                playerPreview.setPosition(new Coordinate<>(1400, 300));
                playerPreview.setAnimationScale(5);
            }
            ...
            case BUTTON_CLICKED -> {
                switch (message.source()) {
                    ...
                    case SAVE_BUTTON ->
                            sceneHandler.notify(new Message(MessageType.SAVE_GAME, ComponentType.SCENE, -1));
            ...
            case WEAPON_IS_SELECTED -> {
                ...
                selectedGun = new ImageWrapper(AssetDeposit.get().getGun(message.source()).getImageWrapper());
                selectedGun.getRectangle().setPosition(new Coordinate<>(1330, 440));
                selectedGun.setScale(5);
                switch (selectedPlayer) {
                    case BIKER ->
                            playerPreview = new Animation(AssetDeposit.get().getAnimation(AnimationType.BikerIdleGun));
                    case CYBORG ->
                            playerPreview = new Animation(AssetDeposit.get().getAnimation(AnimationType.CyborgIdleGun));
                    case PUNK ->
                            playerPreview = new Animation(AssetDeposit.get().getAnimation(AnimationType.PunkIdleGun));
                }
                playerPreview.setPosition(new Coordinate<>(1400, 300));
                playerPreview.setAnimationScale(5);
            }
